...
+		 *
+		 * HMAX is kept conservative at 1108 (same as the 4:3 mode,
+		 * known-good for a narrower output).  Frame clocks drop
+		 * from 1202*5728 = 6.88M (full) to 1108*5716 = 6.33M, a
+		 * ~8% CSI-2 readout speedup at full square resolution.
+		 * If the sensor streams cleanly with HMAX=1108 at this
+		 * width, try dropping to ~900 for a ~25% speedup — that
+		 * extrapolates linearly from the 4:3 mode's HMAX/width
+		 * ratio but has NOT been hardware-verified.
+		 *
